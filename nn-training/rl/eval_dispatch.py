...
def hold_for_local(pending_len: int, reserved: int, gate_set: bool,
                   past_release: bool) -> bool:
...
def run_local_eval_game(bun: str, weights_snapshot: str, stage: int, seed: int,
                        out_dir: Path, max_ticks: int, difficulty: str,
                        timeout_sec: float, wver: str) -> dict:
    ...
    cmd = [bun, "tools/sim/export-eval-game.ts",
           "--weights", weights_snapshot, "--out", str(out_dir),
           "--stage", str(stage), "--seed", str(seed),
           "--difficulty", difficulty, "--max-ticks", str(max_ticks),
...
def dispatch_eval_round(bun: str, rl_path: str, traj_dir: Path, args, cfg: dict,
                        ...
                        local_gate: threading.Event | None = None) -> None:
    ...
    try:
        ...
        reserved = (min(local_slots, total)
                    if (snapshot_path is not None and local_gate is not None
                        and local_slots > 0) else 0)
                ...
                with lock:
                    if ok:
                        ...
                        if attempt < EVAL_TASK_ATTEMPTS and task not in seen:
                            ...
                            _record_agent_meta(meta_path, {
                                "node": nd["id"], "mode": "eval", "it": it,
                                "stage": task[0], "seed": task[1], "ok": False,
                                "reason": err,
                                "ts": time.strftime("%Y-%m-%d %H:%M:%S")})
            ...
            while time.time() < deadline:
                ...
                try:
                    manifest = run_local_eval_game(
                        bun, snapshot_path, task[0], task[1],
                        traj_dir / "local-eval" / f"rl_s{task[0]}_seed{task[1]}",
                        max_ticks=args.max_ticks, difficulty=args.difficulty,
                        timeout_sec=task_timeout, wver=wver)
                    why = dist_common.validate_eval_result(manifest, wver)
                    if why:
                        raise dist_common.DistError(0, why)
        ...
        if snapshot_path is not None and local_slots > 0:
            for _ in range(local_slots):
                threads.append(threading.Thread(target=local_worker, daemon=True,
                                                name="eval-local"))
        for t_ in threads:
            t_.start()
        for t_ in threads:
...
def dispatch_eval_bg(bun: str, rl_path: str, traj_dir: Path, args, cfg: dict,
                     iter_id: str, it: int,
                     rollout_winrate: float | None = None,
                     local_gate: threading.Event | None = None) -> threading.Thread:
    t_ = threading.Thread(target=dispatch_eval_round,
                          args=(bun, rl_path, traj_dir, args, cfg, iter_id, it,
                                rollout_winrate, local_gate),

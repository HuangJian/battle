...
用法（经统一启动器，勿裸跑 python）：
  bun dashboard/src/launch/cli.ts --script scripts/eval_intent_m5.py \
    --data tmp/intent-probe-hard/shards --weights tmp/intent-weights-A.json --out tmp/probe-M5-A-gate.json
  # B 臂（多根合并，与训练同 --data）：
  bun dashboard/src/launch/cli.ts --script scripts/eval_intent_m5.py --data tmp/intent-probe-hard/shards tmp/human-obs --weights tmp/intent-weights-B.json --out tmp/probe-M5-B-gate.json
"""

...
def test_gate_keeps_native_paths_when_wslpath_maps_elsewhere(tmp_path: Path) -> None:
    ...
    env = {
        **os.environ,
        ...
        # 只留 pytest 路径（工具都是假 python，瞬时返回）；跳过 ruff/mypy 免去无关分支。
        "NN_GATE_SKIP": "ruff,mypy",
    }
    proc = subprocess.run(
        ["bash", _bash_path(hook_dir / GATE.name)],
        cwd=skel,
        env=env,
        capture_output=True,
        text=True,
    ...
    assert str(hook_dir / "detach-run.py") in detach, (
        f"detach 脚本路径应是骨架内的 POSIX 绝对路径\n{detach}"
    )
    assert str(fake_py) in detach, f"detach 内层 argv[0] 应是原生 python 的 POSIX 路径\n{detach}"

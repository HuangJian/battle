...
#: 物料落地的**实际调用点**在 `remote/download.py`（S4 第七刀/第十二刀）：`worker` 里那个
#: `download_payload` 只是转发名 ⇒ patch `worker.download_payload` 是**静默空操作**
#: （真 HTTP 打到 `http://hub`，报 502 退避重试，用例变红在面外）。
from remote import download as DL
...
def downloads(monkeypatch: pytest.MonkeyPatch) -> list[str]:
    ...
    monkeypatch.setattr(DL, "download_payload", _spy)  # 见文件头 DL 的说明（调用点在 download）
    return seen

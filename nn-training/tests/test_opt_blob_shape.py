...
★ 2026-09-25（并入 origin 后对齐本地拆分）：这些实现已不在 `remote/worker.py` 一家：
`pack_opt_tar` / `unpack_opt_tar` 住 `remote/job_fs.py`，`_resolve_weights` /
`_cache_produced_weights` 住 `remote/download.py`，产物段（`_cache_produced_weights` 的
调用点）与 W4 restore 分流住 `remote/train_core.py`。三条测试约定因此跟搬：
  · **测试读的实现名**：仍可经 `worker.*` 的显式转发读（那正是门面承诺）；
  · **patch 面**：`_resolve_blob` 调 `download_blob` 是**组内互调** ⇒ 探针必须写在
    `remote.download` 上（patch `remote.worker` 是静默空操作，见 `tests/test_download_split.py`）；
  · **源码锚点**：一条口径跨两个模块的（产物段 + restore 分流）就把两份源码拼起来读。
"""
...
from remote import train_core as train_core_mod
...
def test_resolve_weights_source_priority(tmp_path: Path, monkeypatch) -> None:
    ...
    # patch 面 = `remote.download`（`_resolve_blob` → `download_blob` 是组内互调）。
    monkeypatch.setattr(download_mod, "download_blob", _dl)
...
def test_resolve_weights_transient_is_retryable(tmp_path: Path, monkeypatch) -> None:
    ...
    """
    # patch 面 = `remote.download`（见模块头「patch 面」那一条）。
...
def test_resolve_weights_legacy_tar_without_init_blob(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(download_mod, "download_blob", _400)
...
def test_run_job_refusal_and_order_anchors() -> None:
    ...
    """
    # 这条口径**跨两份源码**：作业壳（`worker`：顺序锚点 + kind=iter 的提前拒绝）与训练核
    # （`train_core`：restore 段的 W4 分流 + 两处拒绝）。拼起来读，才是「一条口径」的全貌。
    src = Path(worker_mod.__file__).read_text(encoding="utf-8")
    core = Path(train_core_mod.__file__).read_text(encoding="utf-8")
    both = src + core
    assert "四源皆尽" in both
    # 三处拒绝都带源清单：① kind=iter/run 的提前拒绝；② 有 tar 但无 model.pt；③ 无 tar 首轮
    assert both.count("{'/'.join(WEIGHT_SOURCES[:4])}") == 3, "三处拒绝都要带源清单"
    assert "WEIGHT_SOURCES: tuple[str, ...] = (" in Path(download_mod.__file__).read_text(
        encoding="utf-8"
    )
    # W4 分流：restore 段按 legacy tar 里有没有 model.pt 决定，并把它记成 legacy_tar
    assert 'weights_src = "legacy_tar"' in core
    assert 'legacy_model = opt_dir / "model.pt"' in core
...
def test_weights_produced_lands_in_blob_cache(tmp_path: Path) -> None:
    ...
    sha = download_mod._cache_produced_weights(blob_root, raw, _log)
    assert sha == _sha(raw)
    assert (blob_root / sha).read_bytes() == raw
    # 二次调用幂等（同 sha 已存在直接返回，不重写）
    assert download_mod._cache_produced_weights(blob_root, raw, _log) == sha
...
def test_run_job_product_section_caches_weights() -> None:
    ...
    产物段随训练核搬到 `remote/train_core.py`（作业壳只留壳），锚点跟着搬 —— 继续读
    `worker.py` 会变成「读一份不再有这段的文件」，断言只会静默空过。
    """
    src = Path(train_core_mod.__file__).read_text(encoding="utf-8")

...
def test_role_gate_lives_in_the_lease_critical_section() -> None:
    ...
    """
    src = (ROOT / "remote" / "hub_server.py").read_text(encoding="utf-8")
...
_EXPECTED_CALL_SITES: dict[tuple[str, str], int] = {
    # `claim_next`（多课程挑选）+ `Hub.claim`（push 派发腿）
    ("remote/hub_server.py", "claim"): 2,
    # `Hub.claim_job`（HTTP `POST /jobs/{id}/claim` 的唯一实现入口）
    ("remote/hub_server.py", "claim_outcome"): 1,
    # HTTP 面本体
    ("remote/hub_server.py", "claim_job"): 1,
    # push 派发
...
def test_claim_paths_are_enumerated_and_all_carry_role() -> None:
    ...
    for rel in ("remote/hub_server.py", "remote/push_dispatch.py"):

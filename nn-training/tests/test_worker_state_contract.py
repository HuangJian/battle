...
`remote/worker.py` 有 ~3300 行 / 60+ 个顶层函数，是 S4 剩下的两个神模块之一。它要拆的
...
**实测清点（AST，①③ 的判据就是它）**——真·模块级可变状态共 6 处，按**宿主**三开
（S4 第四步拆出 `wire`、第五步拆出 `http`，状态都随簇搬迁）：
...
| `_opener` | 懒建单例（`global _opener`） | `http` | `_get_opener` | 否 |
| `_POLL_WARN_AT` | `dict[str, float]`（告警节流 60s） | `http` | `_warn_non_200` | 否（本文件补上） |

`worker` 只做 `from remote.{wire,http} import … as …` 的**显式转发**。于是：

* 转发名与宿主是**同一个对象**（本文件 `test_worker_reexports_are_the_same_objects` 钉住）；
* `_WIRE` / `_BULK` / `_POLL_WARN_AT` 是**原地可变** ⇒ `.clear()` / `.reset()` 从哪个入口进都一样；
* `_BEST_RATE` / `_opener` 是**重绑式**标量（float 不可变 / 懒建单例）⇒ 转发的名字会停在旧值，
  注入点**必须**是宿主模块（`remote.wire._BEST_RATE` / `remote.http` 的相关名字）。
...
HTTP = ROOT / "remote" / "http.py"

#: 状态的**宿主文件**（`worker` 只是转发，不是宿主）。
OWNER_FILES = {"worker": WORKER, "wire": WIRE, "http": HTTP}
#: 顶层**可变容器字面量**绑定（拆走或新增都必须改这里——这是有意的手工清单）。
OWNER_CONTAINERS = {"worker": set(), "wire": {"_WIRE"}, "http": {"_POLL_WARN_AT"}}
#: 函数内以 `global` **重绑**的脚本单例（懒建 / 会话级标量）。
OWNER_GLOBAL_REBOUND = {"worker": {"_ACTIVE_CODE_SHA"}, "wire": {"_BEST_RATE"}, "http": {"_opener"}}
#: 顶层构造的**有状态对象**实例（这里是调度器）。
OWNER_INSTANCES = {"worker": set(), "wire": {"_BULK"}, "http": set()}
#: 结构性豁免：独立引导模块里**同名但不同物**的变量。
STANDALONE_BOOT_EXEMPT = {"remote/tailscale_boot.py": {"_BEST_RATE"}}

_ALL_WATCHED = (
    set().union(*OWNER_CONTAINERS.values(), *OWNER_GLOBAL_REBOUND.values(), *OWNER_INSTANCES.values())
)
#: 转发名必须是**同一对象**的（原地可变）状态。
IDENTITY_STATES = {"worker": {"_WIRE", "_BULK", "_POLL_WARN_AT"}}
...
def _restore_worker_state():
    ...
    wire, poll = dict(wire_mod._WIRE), dict(http_mod._POLL_WARN_AT)
    ...
    http_mod._POLL_WARN_AT.update(poll)
    wire_mod._BULK.reset()
...
def _containers(path: Path) -> set[str]:
    """顶层可变容器字面量（跳过 `__all__` 这类 dunder）。"""
    return {
        name
        for name, value in _top_level_bindings(path).items()
...
def test_mutable_container_bindings_are_exactly_the_pinned_set_per_owner() -> None:
    """顶层可变容器清单按宿主逐一对账（多一个少一个都红）。"""
    for owner, path in OWNER_FILES.items():
        found = _containers(path)
        pinned = OWNER_CONTAINERS[owner]
        assert found == pinned, (
            f"{owner} 顶部可变容器清单变了：\n"
            f"  新增/未登记：{sorted(found - pinned)}\n"
            f"  已消失：{sorted(pinned - found)}\n"
            "拆分时请同步本清单：新增 = 新的共享点；消失 = 先确认没人再依赖它"
        )


def test_global_rebound_singletons_are_exactly_the_pinned_set_per_owner() -> None:
    """函数内以 `global` 重绑的脚本单例按宿主各归各位。"""
    for owner, path in OWNER_FILES.items():
        assert _global_declared(path) == OWNER_GLOBAL_REBOUND[owner], owner


def test_bulk_is_a_stateful_scheduler_instance_built_in_wire() -> None:
...
def test_pinned_state_names_are_all_really_loaded_by_their_host_code() -> None:
    ...
    for owner, path in OWNER_FILES.items():
        pinned = (
            OWNER_CONTAINERS[owner] | OWNER_GLOBAL_REBOUND[owner] | OWNER_INSTANCES[owner]
        )
        assert sorted(n for n in pinned if n not in _loaded(path)) == [], owner


def test_worker_reexports_are_the_same_objects() -> None:
    """**原地可变**的转发必须是同一对象（不是各建一份）——「两份账」故障的正面判据。

    只对 `_WIRE` / `_BULK` / `_POLL_WARN_AT` 成立（`id()` 恒等）。`_BEST_RATE` / `_opener`
    **故意不在其列**：它们会被宿主 `global` **重绑**（float / 懒建单例），转发名会停在旧值
    ——这正是它们注入点必须是宿主模块的原因（见 `tests/test_wire_split.py` 与
    `tests/test_http_split.py` 的注入点口径用例）。
    """
    for name in IDENTITY_STATES["worker"]:
        host = wire_mod if hasattr(wire_mod, name) else http_mod
        assert name in vars(W), f"{name} 不在 remote.worker 的命名空间里"
        assert vars(W)[name] is vars(host)[name], (
            f"remote.worker.{name} 与宿主不是同一对象（转发成了副本 ⇒ 两份账/两个调度器）"
        )
...
def test_no_other_module_owns_a_copy_of_the_worker_state() -> None:
    ...
    """
    owner_rels = {p.relative_to(ROOT).as_posix() for p in OWNER_FILES.values()}
    offenders: list[str] = []
...
def test_poll_warn_throttles_per_url_and_status(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    clock = {"now": 1000.0}
    monkeypatch.setattr(http_mod.time, "time", lambda: clock["now"])
    http_mod._POLL_WARN_AT.clear()
    lines: list[str] = []
    log = lines.append

    http_mod._warn_non_200("http://hub", 403, log)
    assert len(lines) == 1 and "封禁" in lines[0]
    http_mod._warn_non_200("http://hub", 403, log)  # 同 (url, status) 且在 60s 内
    assert len(lines) == 1
    http_mod._warn_non_200("http://hub", 500, log)  # 不同 status ⇒ 另算一条
    assert len(lines) == 2
    clock["now"] += 61.0
    http_mod._warn_non_200("http://hub", 403, log)  # 过窗 ⇒ 再报
    assert len(lines) == 3
    assert set(http_mod._POLL_WARN_AT) == {"http://hub:403", "http://hub:500"}


def test_poll_warn_without_log_is_a_noop(monkeypatch: pytest.MonkeyPatch) -> None:
    """`log=None` 直接返回，且**不**占用节流窗（否则一次静默调用会吃掉一分钟的告警）。"""
    monkeypatch.setattr(http_mod.time, "time", lambda: 5000.0)
    http_mod._POLL_WARN_AT.clear()
    http_mod._warn_non_200("http://hub", 403, None)
    assert http_mod._POLL_WARN_AT == {}


def test_state_shapes_are_pinned() -> None:
    """运行期形状（拆分后逐字段对账的口径）：类型 + `_opener` 的懒建不变量。"""
    assert isinstance(wire_mod._WIRE, dict)
    assert isinstance(http_mod._POLL_WARN_AT, dict)
    ...
    assert http_mod._opener is None or hasattr(http_mod._opener, "open")

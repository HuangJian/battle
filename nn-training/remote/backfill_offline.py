...
# `_weights_archive_root` 随归档那段一起搬进了离线段混入（S4：`hub_server` 收口成门面后
# 自己不再有实现）——它读的仍是同一个 env，而 `_JobStore` 从门面取（名字是契约）。
from remote.hub.store_offline import _weights_archive_root

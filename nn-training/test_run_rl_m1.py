...
def _report_text(total: int, error: int, cleared: int, win_pct: float) -> str:
    ...
    return (f"[m1-eval] WIN RATE {win_pct}% (gate 60%) -> PASS\n"
...
def test_parse_m1_eval_report() -> None:
    ...
    r3 = parse_m1_eval_report("[m1-eval] win rate 55.5% (gate 60%) -> FAIL\n"
...
def test_run_clean_eval_rerun() -> None:
    ...
    calls.clear()
    res = run_clean_eval("bun", "tmp/w.json", args,
    ...
    check(n_calls[0] == 2 and res["error"] == 0 and res["retries"] == 1
          and res["cleared"] == 262, f"error 后自动重跑至干净 (calls={n_calls[0]})")
    calls.clear()
    res = run_clean_eval("bun", "tmp/w.json", args,
                         _runner=fake_clean(_report_text(350, 350, 0, 0.0)))
    check(len(calls) == CLEAN_EVAL_MAX_RETRY and res["error"] > 0
...
def test_run_clean_eval_goal_cmd() -> None:
    ...
    check("--policy intent-exec" in captured[-1] and "--intent-weights" in captured[-1],
          "intent 用 intent-exec + --intent-weights")
    captured.clear()
    args.goal = True
    run_clean_eval("bun", "tmp/g.json", args, _runner=runner)
    check("--policy goal" in captured[-1] and "--goal-weights" in captured[-1],
...
def test_resolve_mode() -> None:
    ...
    check(run_rl.resolve_mode(["--goal", "--mode", "intent"]) == "intent",
          "--mode 后出现者胜")
...
def test_apply_mode_flags() -> None:
    ...
    check(args.intent_rollout and not args.goal_rollout and not args.goal,
          "intent：intent_rollout=True 其余 False")
    args = types.SimpleNamespace(mode="per-tick", goal=False)
    run_rl.apply_mode_flags(args)
    check(not args.intent_rollout and not args.goal_rollout and not args.goal,
          "per-tick：三 flag 全 False")
    args = types.SimpleNamespace(mode="per-tick", goal=True)
    run_rl.apply_mode_flags(args)
    check(args.mode == "goal" and args.goal_rollout and args.goal,
...
def test_merged_mode_args() -> None:
    ...
    check(merged["a"] == 1 and merged["b"] == 3 and merged["c"] == 4,
          "intent_rl 遗留块覆盖 rl 同名键")
...
def test_stop_loss_hit() -> None:
    print("[fast] stop_loss_hit：止损判门分模式（原 iter15 Δ≤0 泛化）")
    check(run_rl.stop_loss_hit("per-tick", 15, 0.0, 20, {"delta": -0.1}) is False,
          "per-tick 永不触发")
    check(run_rl.stop_loss_hit("intent", 0, 0.0, 20, {"delta": -0.1}) is False,
          "stop_loss_at=0 关闭")
    check(run_rl.stop_loss_hit("intent", 15, 0.0, 10, {"delta": -0.1}) is False,
          "未到 stop-loss-at 不触发")
    check(run_rl.stop_loss_hit("intent", 15, 0.0, 20, {"delta": -0.05}) is True,
          "Δ<=0 触发")
    check(run_rl.stop_loss_hit("intent", 15, 0.0, 20, {"delta": 0.1}) is False,
          "Δ>0 不触发")
    check(run_rl.stop_loss_hit("goal", 15, 0.0, 20, {"delta": 0.0}) is True,
          "goal Δ==0 触发")
    check(run_rl.stop_loss_hit("intent", 15, 0.0, 20, None) is False,
          "无 eval 记录不触发")


def test_update_kwargs() -> None:
    print("[fast] update_kwargs：value 预热 + kickstarting KL 系数衰减")
    args = types.SimpleNamespace(epochs=4, warmup_iters=2, kickstart_kl=1.0,
                                 kickstart_decay=0.5, seed=7)
    kw = run_rl.update_kwargs(args, 1, 1, object())
    check(kw["value_warmup_epochs"] == 4 and kw["seed"] == 7
          and kw["kl_coef"] == 0.0, "warmup：value-only + kl=0")
    kw = run_rl.update_kwargs(args, 3, 1, object())
    check(kw["value_warmup_epochs"] == 0 and abs(kw["kl_coef"] - 1.0) < 1e-9,
          "策略迭代 kl=基础系数 1.0")
    kw = run_rl.update_kwargs(args, 4, 1, object())
    check(abs(kw["kl_coef"] - 0.5) < 1e-9, "kl 每策略迭代衰减 0.5")
    args2 = types.SimpleNamespace(epochs=4, warmup_iters=1, kickstart_kl=0.0,
                                  kickstart_decay=0.5, seed=7)
    check(run_rl.update_kwargs(args2, 3, 1, object())["kl_coef"] == 0.0,
          "kickstart_kl=0 关闭")

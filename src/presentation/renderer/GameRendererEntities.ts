// ================================================================
// EntityRenderSlice — extracted from the former GameRendererEntities.ts mixin
...
export class EntityRenderSlice {
  constructor(private r: GameRendererCore) {}
  // ---- Tanks ----

  renderTanks(world: World, tanks: Tank[]): void {
    ...
    for (let ti = 0; ti < tanks.length; ti++) {
      ...
      const vc = this.r.animations.get(tank.id)
      const animFrame = vc ? this.r.animations.getFrame(vc) : (frame >> 2) & 1
  ...
  renderBullets(world: World): void {
    const artist = this.r.artist
  ...
  renderPowerUps(world: World): void {
    const frame = world.frame
    const artist = this.r.artist

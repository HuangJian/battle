...
/**
 * Standard-mapping button indices (W3C mapping="standard", §354) — the
...
/**
 * Rebindable gamepad actions (§354a follow-up) — the 4 d-pad directions plus
...
/** Default gamepad bindings: the standard-mapping indices (§354a). */
...
export function loadSettings(): GameSettings {
  ...
  try {
    ...
    if (raw) {
      ...
      // Legacy saves predate keys2 (two-player §353): `{ ...saved.keys2 }` on
      ...
      const merged = {
        ...
        // Legacy saves predate pads (§354a follow-up): spreading undefined

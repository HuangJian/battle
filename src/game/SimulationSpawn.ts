// ================================================================
// SpawnSystem — extracted from the former SimulationSpawn.ts mixin
...
import type { IntelligenceLevel, Tank } from '../types'
...
export class SpawnSystem {
  ...
  updateSpawning(): void {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const tier = this.rollSpawnTier(w)
      this.applySpawnTier(w, tank, tier)
  ...
  /**
   * Spawn-time tier roll (plan §5): commander floor guarantee first, then a
   * natural tier roll. A natural commander roll counts against an outstanding
   * floor only while it is still unsatisfied — the counter clamps at 0 (never
   * negative) once satisfied; extra natural commanders beyond the floor are
   * bonus, not debt.
   */
  private rollSpawnTier(w: World): IntelligenceLevel {
    ...
    const tier = rollTier(w.difficultyKey, w.rng)
    if (tier === 'commander' && w.commanderQuotaRemaining > 0) w.commanderQuotaRemaining -= 1
    return tier
  }

  /**
   * Apply a rolled tier to a freshly created enemy tank: alive-cap downgrade
   * check, conditional +15% elite boost, and brain stamp.
   */
  private applySpawnTier(w: World, tank: Tank, tier: IntelligenceLevel): void {

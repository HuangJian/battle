// ================================================================
// EnemiesSystem — extracted from the former SimulationEnemies.ts mixin
...
/**
 * the allied guard system (天降神兵, §31 Phase 2),
...
export class EnemiesSystem {
  ...
  updateGuards(): void {
    ...
    for (let ai = 0; ai < allies.length; ai++) {
      ...
      if (g.guardExpireFrame !== undefined && w.frame >= g.guardExpireFrame) {
        ...
        this.d.effects.createExplosion(g.x + g.w / 2, g.y + g.h / 2, 'big')
        continue
      }
      ...
      if (brain.isFiring()) this.d.combat.tryFire(g)
      brain.endFrame()
    }
  }
   ...
   * memory bounded).
   */
  guardAIById = new Map<number, GodAIInput>()
  ...
  private guardAIFor(g: Tank): GodAIInput {
    ...
    const rng = new RNG(((this.d.world.frame * SEED_HASH) >>> 0) ^ (id * SEED_HASH))
    brain = new GodAIInput(this.d.world, GUARD_GOD_AI_PARAMS, rng, (w) => {
  ...
  private updateGuardYield(g: Tank, brain: GodAIInput, fwd: Direction, allTanks: Tank[]): void {
    ...
    if (
      ...
      g.dir = moveDir
      this.d.combat.tryFire(g)
    } else {
      g.dir = fwd
      if (brain.shouldFireInDir(gcx, gcy, fwd)) this.d.combat.tryFire(g)
      g.dir = moveDir
    }
    g.moving = true
  }
  ...
  updateEnemyAI(): void {
    ...
    this.d.ai.update(this.d.world, (tank) => this.d.combat.tryFire(tank))
  ...
  private updateDecoyEngagement(): void {
    ...
    for (let i = 0; i < tanks.length; i++) {
      ...
      if (chosen) {
        t.dir = chosen
        this.d.combat.tryFire(t) // cooldown-gated; never double-fires
      } else {

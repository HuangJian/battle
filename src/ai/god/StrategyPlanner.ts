...
function intentRead(
  ...
  if (w.frame % INTENT_REVALIDATE_EVERY === 0) {
    const cd = enemyDeadline(w, t)
    const committedDeadline = cd.enemyDamageDeadline
    const currentSlack = committedDeadline - killAssessment(w, p, t).playerKillEta
    // Missed-deadline release applies ONLY to direct threats (csb/cbr): for
    // those the deadline is the real first-shot time, so a negative slack
    // means continuing the hunt eats base damage. Walk-branch deadlines are
    // optimistic lower bounds (enemy charged zero turn costs) — a distant
    // committed enemy there has naturally negative slack at commit, and a
    // late kill still prevents the shots after the first; only the collapse
    // check below may release it.
    if (cd.directThreat && currentSlack < 0) return null

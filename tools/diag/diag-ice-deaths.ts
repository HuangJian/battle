#!/usr/bin/env bun
/**
 * diag-ice-deaths.ts — Was the player on ice when it died? (S18 family)
 *
 ...
 *
 * Usage: bun tools/diag/diag-ice-deaths.ts <stageIdx> <seedList> [--params <file>]
 */
...
const stageIdx = parseInt(process.argv[2] ?? '18', 10)
  ...
  while (tick < 18000 && world.state === 'playing') {
    ...
    for (const e of world.consumeEvents()) {
      if (e.type === 'tank_destroyed' && e.tank.isPlayer) {
        ...
        console.log(
          `S${stageIdx} seed ${seed} death at t${tick}: onIce=${wasOnIce} glide=${glide} cell=(${Math.floor((e.tank.x + e.tank.w / 2) / 16)},${Math.floor((e.tank.y + e.tank.h / 2) / 16)}) lives=${world.lives}`,
        )
      }
    }
  }
  console.log(`S${stageIdx} seed ${seed}: end=${world.state} kills=${world.killCount} t=${tick}`)
}

...
while (tick < 36000) {
  ...
  const aggSuppress = input._aggCampTrack.suppress
  const aggCampTicks = input._aggCampTrack.ticks
  const aggNavStuckTicks = input._aggNavTrack.ticks
  const aggNavSuppress = input._aggNavTrack.suppress
  const navStuckTicks = input._navStuckTrack.ticks
  const navStuckSuppress = input._navStuckTrack.suppress

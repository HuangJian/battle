#!/usr/bin/env bun
/**
 * export-intent-labels-worker.ts — 机械意图 tagger 的池工作进程
 * （plan/Intent-Policy-NN-Plan.md §3.6；由 export-intent-labels.ts 派生）。
 *
...
function processJob(payload: TaggerPayload, job: TaggerJob): TaggerAggregate {
  ...
  for (let t = 0; t < maxTicks; t++) {
    ...
    if (n >= 0) encoder.encode(second.world)
    second.sim.tick()

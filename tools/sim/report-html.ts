...
import type { WinrateSnapshot } from './winrate-history'
...
export function mean(n: number, d: number): number {
  return d > 0 ? n / d : 0
}
...
// ----- compact run shape embedded into the HTML payload -----
...
export function compactCurrent(aggs: DiffAgg[], ctx: ReportMeta): CompactRun {
  ...
  return {
    ...
    at: fmtTime(ranAt),
    git: '',
    seeds: ctx.seedsCount,
    stageCount,
    diffs,
  }
}

export function compactSnapshot(s: WinrateSnapshot, ctx: ReportMeta): CompactRun {
  const diffs: Record<string, CompactDiff> = {}
  ...
  const ranAt: string = s.generatedAt || s.savedAt
  void ctx
  return {
    id: s.id,
    label: s.label,
...
<h1>God AI 胜率扫描报告${meta.modeSuffix}</h1>

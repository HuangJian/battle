 ...
 *
 * Usage
 *   bun tools/eval-suite.ts                       full 35-stage scorecard
 *   bun tools/eval-suite.ts --seeds 60            more seeds (see §6: ≥60 to conclude)
 *   bun tools/eval-suite.ts --stages 6,18,32      subset
 *   bun tools/eval-suite.ts --params best.json    score a tuned parameter set
 *   bun tools/eval-suite.ts --dims                per-dimension breakdown
 *   bun tools/eval-suite.ts --weights             nominal vs effective weight audit
 *   bun tools/eval-suite.ts --calibrate           regenerate tools/eval-refs.json
 *   bun tools/eval-suite.ts --compare a.json b.json    paired A/B under CRN
 *   bun tools/eval-suite.ts --json out.json       machine-readable dump
 *
  ...
  try {
    ...
    if (flag('calibrate')) {
      ...
      const out = {
        ...
        source: {
          tool: 'tools/eval-suite.ts --calibrate',
      ...
      console.log(
        '\nWeights are NOT applied automatically. Copy the ADOPT column into\n' +
          'DEFAULT_LOSS_WEIGHTS in tools/godai-score.ts only after reviewing the fit.',
      )
      return
    }

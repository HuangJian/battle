/** Hero.tsx — 训练状态 hero（一屏焦点）：胜率大数字 + 八格走势图
 *  （行1 = rollout 采样：胜率/承伤·杀/击杀；行2 = eval 胜率 + 胜局耗时/胜局残血；行3 = 败局耗时/道具；
 *  除 eval 胜率外均为 rollout「所有 iter 平均」口径）；
      ...
      <div className="tc-hero__right">
        ...
        <div className="tc-trends">
          ...
          <TrendCell
            ...
            range={range}
            yFloor={0}
            title="每杀承伤 = 全样本总承伤 / 总击杀（rollout 所有 iter 平均；越小越会周旋；防苟活：不打则分母小、值爆炸）"
          />
          <TrendCell
            series={killsSeries}
            fmt={(v) => (v != null ? `${v.toFixed(1)}` : '—')}
            ...
            title="每局平均击杀"
          />
          {/* 行2：eval 胜率 + 胜局耗时/胜局残血（后两者 rollout 所有 iter 平均） */}
          <TrendCell
            series={evalSeries}
          ...
          <TrendCell
            ...
            fmt={fmtInt}
            range={range}
            title="胜局平均耗时（ticks，仅胜局计入，rollout 所有 iter 平均）"
          />
          <TrendCell
            series={winHpSeries}
            fmt={fmtInt}
            range={range}
            yFloor={0}
            title="胜局平均剩余 hp（rollout 所有 iter 平均；剩余命每命计满额）"
          />
          {/* 行3：败局耗时 / 道具 */}
          <TrendCell
            ...
            fmt={fmtInt}
            range={range}
            title="败局平均耗时（ticks，仅败局计入，rollout 所有 iter 平均）⚠️ 越高不代表越强——同构关里「活得久」往往是打不死敌人的症状（清场停滞），须与胜率并排读"
          />
          <TrendCell
            series={puSeries}
            fmt={(v) => (v != null ? `${v.toFixed(1)}` : '—')}

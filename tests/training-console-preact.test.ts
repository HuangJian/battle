...
describe('MultiCourseOverview 同屏多课总览（P5-W2）', () => {
  it('单课不渲染；多课每课一行（组件状态点 + 最近指标 + 停机徽标）', async () => {
    const { MultiCourseOverview } = await import(
      '../tools/training/console/ui/panels/MultiCourseOverview'
    )
  ...
  it('getCourseOverviews：空课程清单 → 空数组（不碰账本/日志）', async () => {
    const { getCourseOverviews } = await import('../tools/training/console/api')

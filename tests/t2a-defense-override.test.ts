...
describe('§159: T2a defense override for close enemies', () => {
  ...
  it('with fix (range=4): player fires at close enemy despite being past threshold', () => {
    ...
    expect(input._moveDir === 'left' || (world.player!.dir === 'left' && input._moveDir === null)).toBe(true)
  })
  ...
  it('end-to-end: S20 Bastion seed 383912762 — fix increases player fire count', () => {
    // Full simulation reproduction: the fix should increase the player's fire
    // count by tick 2400 (the oscillation period) because the player is no
    // longer stuck in a zero-fire up/down oscillation.
    const { STAGES } = require('../src/config/stages')
    const { DIFFICULTIES } = require('../src/config/difficulty')
    const { RULES, DEFAULT_RULES } = require('../src/config/rules')
    const seed = 383912762
    const stageIdx = 19 // S20 Bastion (0-based)

    function runSim(overrideRange: number): { fireCount: number; killCount: number } {
      const world = new World()
      ...
      const params = { ...DEFAULT_GOD_AI_PARAMS, t2aDefenseOverrideRange: overrideRange }
      ...
      return {
        fireCount: world.player?.fireCount ?? 0,
        killCount: world.killCount,
      }
    }

    // The fix should increase the fire count (player is no longer stuck
    // in a zero-fire oscillation) and the kill count (player kills enemies
    // it was previously ignoring).
    const before = runSim(0)
    const after = runSim(4)
    expect(after.fireCount).toBeGreaterThan(before.fireCount)
  })
})

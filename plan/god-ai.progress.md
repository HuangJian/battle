...
## Round 5 — S32 Diamond 贴身缠斗策略（2026-07-29，DECISIONS §43）

### 目标与最终成绩

| 目标 | 结果 | 判定 |
|------|------|------|
| S32 通关率 > 80% | **72.5%** @120 seeds（85% @20 seeds） | ⚠️ 接近但未达 |
| 整体均值不下降 | **86.9%** @20 seeds（+5pp from 81.9%） | ✅ PASS |
| 全 35 关 > 60% floor | **35/35 达标**（S32 从 52%→72.5% 脱离 floor） | ✅ PASS |

### 核心策略：贴身缠斗（t2aMaxRange=2）

用户脑洞策略的核心洞察：重甲坦克 4 血，远程对射时炮弹飞行时间长（15 格 = 1s），火力频率极低（4 枪/4s）。贴身 2 格时炮弹飞行 ≈ 0，火力频率 ≈ 0.13s/枪，4 血重甲 0.5s 击毙——8 倍效率提升。

**新增参数** `t2aMaxRange`（默认 15 = AIM_RANGE_CELLS，不改变其他 34 关行为）：
- T2a（stop-and-aim）分支仅当敌人在 `t2aMaxRange` 格内时触发
- 超出范围时玩家继续移动（directMove/A*）拉近距离，到 2 格才停车射击
- S32 override 设 `t2aMaxRange: 2`

**辅助参数**（均仅 S32 override）：
- `campTimeoutTicks: 50`（原 90）— 缩短无效驻扎
- `antiCampSuppressTicks: 50` — 匹配抑制窗口
- `damagedArmorBonus: 1` — 优先补刀受伤重甲（伤害永久）
- `navStuckTicks: 90`（原 180）— 更快脱困

### 否决方案（全部 ≥60 seeds 验证）

| 方案 | 结果 | 否决原因 |
|------|------|----------|
| 守卫带（被动占位） | 7% | 玩家坐固定格，敌人绕行 |
| 基地中心目标选择 | 32% | 拉向远处威胁，忽略近敌 |
| T2a 跳过（全威胁） | 28% | 中断重甲击杀，死亡翻倍 |
| T2a 跳过（仅快车） | 50.8%@120 | 仍打断重甲效率 |
| maxPlayerDistFromBase 缩小 | 24-48% | 限制移动→杀敌不足 |
| aimError=0 | 47% | 去除打破僵局的微噪声 |
| damagedArmorBonus>1 | =1 | 不改变目标排序 |

### 验证

- `bun run check`：530 pass / 0 fail，tsc clean，oxlint clean
- Parity baseline re-locked（seed 42：2355→2194 ticks, 3→4 lives — 改善）
- 35×20 seeds 终审：mean 86.9%，0/35 below 60% floor

### 剩余差距

72.5%→80% 的差距在 base_destroyed 尾部（21/120 = 17.5%）。快车突破开阔底带时，玩家仍在远处贴身缠斗重甲，来不及回防。已尝试的所有回防手段（守卫带、T2a 跳过、参数限制）都会打断重甲击杀效率，导致净负收益。后续可探索 D3（迷宫感知导航）或主动防御基地墙。
...
## Round 4（已被执行）— S32 Diamond 攻坚方向论证

> 本轮方向已在 Round 5 中执行完毕，详见上方 Round 5 记录。D1（守卫带）和 D2（重甲优先级）均经测试否决；用户脑洞策略“贴身缠斗”（t2aMaxRange=2）成为最终方案。

...
## 5.6 B5：`BatchEvalRunner._run`（821 行）的相位切分 —— B5a 已落地，B5b 待做

§5.5.4 把 B5 标成「另开一轮」（`_run` 的 821 行按阶段切）。这里是那一轮的现状量化、已落地的
B5a（相位边界）、以及 B5b（状态对象化）的设计。**零行为改动的纪律与 B1/B3 相同**：先量后切，
每一段都要能「逐字节对账」或「差分探针等价」。

### 5.6.1 现状量化（2026-09-25 侦察，脚本 `nn-training/tmp/recon_b5*.py`）

`_run` = **821 行**，内部只有 **11 个嵌套 def**（`lane_state` / `bringup` / `mark_tripped` /
`spawn_workers` / `supervise` / `window_open` / `record` / `params_for` / `fetch_manifest` /
`worker` / `_watch`），其余全是**顺序语句**：

| 段 | 行区间 | 行数 | 内容 |
|---|---|---|---|
| A 单元开头 | 153–271 | 119 | 配置 / god 分支 / iterId / 语料 pairs / 权重快照 / 通道表；**两处早退**（缺权重、已全结算） |
| S 通道机器 | 273–887 | 615 | 状态块（29 个可变容器 + 2 锁 + 2 Event）→ 11 个闭包 → 起跑 → 主循环三闸 |
| D 收尾 | 888–972 | 85 | 停机 / 断连 / 只等正在写行的赢家 / join / 落账 / 心跳 / 返回 |

**关键量化（决定「能不能切」的）**：

- `_run` 外层直接赋值 **65** 个名字，其中 **58** 个被机器段读；机器段对这些名字有 **212 处引用**。
- 其中 **139 处在 11 个闭包内部**（`worker` 77 · `bringup` 19 · `spawn_workers` 0 ……）。
- **零 `nonlocal`** —— 状态全靠**可变容器**（`seen: set` / `settled: [0]` / `lanes: dict`）来绕过
  闭包只读的限制。这正是「切不动」的根因：**不是没有链，是状态没有所有者**（与 B2 同型）。
- 闭包之间互相调用（`supervise` → `bringup` / `window_open` / `spawn_workers`；`worker` → `record` /
  `params_for` / `fetch_manifest`），所以「按调用图切」在这里只能切出一个 500 行的连通分量。

### 5.6.2 ✅ B5a（2026-09-25，第三十刀）：相位边界，机器体**逐字节不动**

| 成员 | 行数 | 职责 |
|---|---|---|
| `_run` | 6 | 只讲相位：`plan = _open_unit()` → 早退透传 → `_run_channels(plan)` |
| `_open_unit` | 158 | 单元开头；产出 `_UnitPlan`（**29 字段的对外契约**） |
| `_run_channels` | 677 | 状态 + 11 个闭包 + 主循环（**原 615 行机器体逐字节保留**）+ 取值段 |
| `_settle_unit` | 87 | 收尾三闸 + **对台账的唯一交代**（`mark_unit_done` / `reopen_for_resume`） |
| `_log_provenance` | 47 | 参与度账（谁跑的必须自证 + 两条响亮告警） |

**为什么这样切**：三段的边界是**相位**（谁的失败模式是什么、谁对台账负责），不是行数。
`_UnitPlan` 只收「机器与收尾真要用到的」29 个名字 —— 单元开头内部的中间量
（`policy_cfg` / `window` / `god` / `iter_base` / `pairs` / `done_before` / `snapshot_path`）**不出界**，
否则契约会退化成「什么都依赖」。

**验证（可复核）**：

- **逐段字节对账**（`tmp/verify_b5.py`，基准 = `git show HEAD:`）：A 119 行 · S 615 行 ·
...
- **★ 新获得的可测性**（拆分前做不到）：`_settle_unit` / `_log_provenance` 现在**直接可调** ——
  ...
  「任何失败只记日志绝不抛出」、两条响亮告警、两处短路，都成了单测（10 例）。
- 守卫演进 2 处：`test_batch_runner_split` 的 import 闭集（+`typing`）·
  `test_batch_plan_split` 的入边归属者改名（`_run.bringup` → `_run_channels.bringup` 等）。
...
> 11 个闭包逐节点等 · 主循环 6 + 收尾 1），跨版本差分由「非结构用例两版全绿」承担（改动测试文件仅 4 个）。

**目标形态**：一个类（`_UnitLanes`）持有那 **58 个名字**为属性，
11 个闭包变成方法（`bringup` / `supervise` / `worker` / `record` / `fetch_manifest` / `params_for` /
`lane_state` / `mark_tripped` / `spawn_workers` / `window_open` / `_watch`），
`_run_channels` 退化成「构造 → `lanes.run()`」；三闸与收尾留在 `_settle_unit`。

**为什么它值一做**：677 行的方法里，读者要同时装下 29 个状态字段、11 个闭包的作用域、以及
三闸的时序；`worker`（180 行）本身就是第二层巨块。状态对象化之后每个方法一个判据。

**代价与风险（写明）**：

- 这是**改写**不是搬运：~212 处引用要变成属性访问（其中 139 在闭包内），闭包内的 40 处 `self.*`
  （现在指**执行器**）要改成对执行器的显式回指。**字节对账在这里不成立** ⇒ 主证据必须是别的。
- 并发代码改写错 ⇒ 「评估行错但看着正常」，是本仓最贵的一类错。因此**验证方案先定死**：
  1. **AST 变换证明**：新体 == 旧体施加**确定性映射**（`{状态名: self.名}` + `self.` → `self.owner.`）
     后的结果（逐方法、逐语句比对；不匹配就红）。
  2. **差分探针**：同一 fixture（本机槽位跑真单元 + 假节点）分别打在 `git show HEAD:rl/batch_runner.py`
     与新实现上 ⇒ 比对**返回 dict + 逐局行 jsonl 逐字节 + 日志行序列**（含背压 / 竞速重复 /
     部分完成 / 零消费者 deferred / 收尾僵死五条路径各一遍）。
  3. 全量门禁 + 既有 e2e（`test_dual_track_eval` 等）。
- **不做的事**：不改通道语义（每节点各自就绪即派单 / 失联重探 / 尾段竞速）——那是**任务侧**的裁定
  （用户 2026-09-19 五条），本步只挪代码；不引入线程池 / waitgroup 之类的新抽象。

**进入条件**：本轮先把 B5a 的收益吃干净（`_settle_unit` / `_log_provenance` 的守卫与用例站稳），
再开 B5b；两刀分开提交，任一刀可独立回滚。

### 5.6.4 B5b 之后：余下候选（**本轮不做**，留档供下轮判据）

B5a + B5b 之后 `_run` 这条链已收口（相位 6 / 开头 158 / 机器 675 类内 13 方法 / 收尾 87 / 账 47）。
若还要往下切，**判据仍然是「有没有独立的所有者 + 独立触发条件」**，而不是行数：

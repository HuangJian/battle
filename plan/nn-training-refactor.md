...
| B5 | **另开一轮**：`_run` 的 821 行按阶段切（通道机器 ~500 / 收尾 ~80 / 参与度账 ~60 / 单元开头 ~60） | —— | 不在本接口范围；**已开轮 → 见 §5.6**（B5a ✅ 相位边界已落地，B5b 状态对象化待做） |
...
### 5.6.3 B5b（待做）：状态对象化 —— 把 677 行拆成 ~8 个方法

**目标形态**：新模块 `rl/batch_lanes.py`，一个类（暂名 `UnitLanes`）持有那 **58 个名字**为属性，

...
> **最新状态（2026-07-29 晚，P4 完成）**：全 35 经典关目标 **基本达成** —— 真值口径
> （35 关 × 60 seeds，classic，18000t，逐关覆盖表生效）：
> - **平均过关率 81.9%**（目标 >80% ✅ PASS）
> - **34/35 关 ≥ 60% floor**；唯一低于 floor：**S32 Diamond 52%**（已判定为结构性硬关，
>   见下方 S32 备注，参数空间已穷尽）
> - 机制：CMA-ES R7 全局最优（`DEFAULT_GOD_AI_PARAMS`）+ **逐关参数覆盖表**
>   （`src/ai/godai-stage-overrides.ts`，data over code）：S6/S18/S25/S26 由覆盖表拉回
>   floor 之上（57→63 / 52→67 / 57→73 / 53→65）
> - 全套件 530 测试通过；parity 基线已重锁；回归门禁重写为全 35 关（~13s）
>
> **S32 Diamond（结构性未解，P5 候选）**：8 armor + 8 fast + 4 power 的重甲配置 +
> 碎片化钢墙/森林迷宫 + 基地底部开阔带。60-seed 下所有单/双参数改动以及单关专属
> CMA-ES（30-seed 内环 60% 在 60 fresh seeds 复测仅 43%）全部 ≤ BASE。需要结构性
> 方案：迷宫感知导航，或重甲关专用基地近卫行为。

## 进度总览

| 阶段 | 状态 | 结果 | 关键改动 |
|------|------|------|----------|
| P0 — T2a 死锁修复 | ✅（aec21f4） | S0 20%→70%，S1 22.5%→87.5% | `scan.enemy` 门 + anti-camp 逃逸 + nav-stuck 逃逸 |
| P1 — 生存与防御加固 | ✅（2cedb7a） | S0 70%→87.5%，S1 87.5%→92.5% | 闪避加宽 + 基地威胁 `row>=18` + 终局回防 + 跳过 T2a/道具 |
| P2 — 反驻扎区域 + 预测瞄准 | ✅ | S0 86.7%, S1 100%, S3 50%→66.7% | 反驻扎区域跟踪（±1格）+ nav-stuck 任意方向逃逸 + 预测瞄准 |
| P3 — CMA-ES 多关优化 | ✅ | 4 关聚合 63%→75% | v4.1 fitness，S0/S3/S6/S9 |
| P4 — 全 35 关 floor-aware 优化 | ✅（本轮） | **mean 81.9%，34/35 ≥60%**（35×60 真值） | 7 轮 IPOP CMA-ES（worker pool ×15）+ race-to-base + outnumbered-retreat + **逐关覆盖表** + 60-seed 验证协议 |
| P5 — S32 结构性修复 | ⏳ 候选 | 待定 | 迷宫感知导航 或 重甲关基地近卫 |

**P4 方法论结论（对后续轮次有约束力）**：
1. **单一全局参数组无法满足全部 35 关** —— 弱关失败家族的需求互斥（S6 要禁回撤，
   S18 要更早回撤）。解法 = 逐关覆盖表（按关卡名 keyed），每条覆盖必须 ≥60 seeds
   对照验证。
2. **20-seed 探针只能筛选、不能定论**（二项噪声 ±11pp，会选中海市蜃楼收益）；
   所有决策性测量一律 60 seeds。
3. **CMA-ES 内环必须 = 全 35 关**（聚焦弱关的内环必然过拟合，R5 已证明）。
4. 单关专属 CMA-ES 对结构性硬关无效（S32：30-seed 内环 60% → 60 fresh seeds 43%）。

> 回归门禁（`tests/god-ai-regression-gate.test.ts`）已重写为**全 35 关 × 20 seeds**，
> 逐关 floor = 60-seed 真值 − 4 wins 余量，另有聚合 77% floor（~13s，纯确定性）。
> 旧版只测 S0/S1，曾静默掩盖 S1 回归——不得回退到窄门禁。

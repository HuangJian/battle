/** evalboard-requests.test.ts ↔ dashboard/src/evalboard/requests.ts（§5.3 请求文件协议）。 */
...
describe('写者唯一性（P1：console/CLI 只 append 请求，runner 单写台账）', () => {
  it('backfill CLI 改道 appendRequest，不再直写 batches.jsonl', async () => {
    ...
    const src = await Bun.file(
      path.join(import.meta.dir, '..', 'src', 'evalboard', 'backfill.ts'),
    ).text()
...
describe('P1 写者唯一性（触发端只写请求文件）', () => {
  it('console / ui / backfill 均无台账写调用', () => {
    ...
    const dirs = [path.join(root, 'src', 'server'), path.join(root, 'src', 'web')]
    const files = dirs.flatMap((d) => tsFiles(d))
    files.push(path.join(root, 'src', 'evalboard', 'backfill.ts'))

...
import {
  ...
  summarizeSmoke,
  type SmokeItem,
} from '../src/stack/smoke'
import { resolveTorchThreads, torchThreadEnv } from '../src/core/venv'
import { resolveTrainScript, parseCli } from '../src/launch/cli'
import { aggregateNodeHistory, emptyHistory, poolStatus } from '../src/server/pool-history'
import { readIterMetrics } from '../src/server/iters'
import { stripIsoPrefix } from '../src/web/view'
import { TRAINING_LOOP_ENTRY, trainingLoopSpec } from '../src/stack/specs'
...
describe('training smoke suite (dashboard/src/smoke.ts)', () => {
...
describe('training venv helpers (dashboard/src/venv.ts)', () => {
...
describe('pool data layers (dashboard/src/server/pool-history + iters)', () => {
...
describe('train CLI arg parsing (dashboard/src/launch/cli.ts, DECISIONS §349)', () => {
  ...
  it('--help exits 0 with usage', () => {
    const r = Bun.spawnSync(['bun', 'dashboard/src/launch/cli.ts', '--help'], {
      cwd: REPO_ROOT,
      stdout: 'pipe',
      stderr: 'pipe',
    })
...
describe('supervisor sentinel fingerprint', () => {
  it('detects sentinel changes via mtime', async () => {
    const { sentinelsChangedSince, sentinelStamp, lastMonitorChange, monitorTouch } =
      await import('../src/core/sentinels')

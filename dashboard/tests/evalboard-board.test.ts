/** evalboard-board.test.ts ↔ dashboard/src/server/eval-board.ts（P3 看板端到端合成）。 */
import { describe, expect, it, beforeAll, afterAll } from 'bun:test'
import { mkdtempSync, readFileSync, rmSync, writeFileSync } from 'fs'
import { tmpdir } from 'os'
import path from 'path'
import { appendRow, type EvalGameRow } from '../src/evalboard/store'
...
describe('看板合成端到端', () => {
  it('入队 → 400 局入账 → 过门 + S1 告警', () => {
    ...
    // God 基线工作副本（读入库正本：绝对路径，与 cwd 无关）。
    const lad = JSON.parse(readFileSync(LADDER_CANON_PATH, 'utf-8')) as {

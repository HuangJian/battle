...
describe('cardFamilies：服务面 vs 课程面', () => {
  it('分族与族内顺序：服务面（agent → hub → 隧道）在前，课程面（trainer → 本机 worker）在后', () => {
    ...
    const groups = cardFamilies([
      ...
      cv('trainingLoop', 'course'),
      cv('hubServer', 'shared'),
      cv('selfNode', 'singleton'),
    ])
    expect(groups.map((g) => g.id)).toEqual(['service', 'course'])
    expect(groups[0]!.rows.map((r) => r.key)).toEqual(['selfNode', 'hubServer', 'cloudflared'])
    expect(groups[1]!.rows.map((r) => r.key)).toEqual(['trainingLoop', 'localWorker'])
  ...
  it('scope 缺省/未知 ⇒ 课程面（单侧保守：少一个徽章只是少信息，空贴「共享」是假承诺）', () => {
    const groups = cardFamilies([cv('hubServer'), cv('trainingLoop')])
    expect(groups.map((g) => g.id)).toEqual(['course'])
    expect(groups[0]!.rows.map((r) => r.key)).toEqual(['trainingLoop', 'hubServer'])
    expect(scopeBadge(cv('hubServer'))).toBeNull()
  })

  it('未列进化妆顺序的 key 落组尾但**不丢**（成员资格只由 scope 决定）', () => {
    const groups = cardFamilies([cv('someNewThing', 'course'), cv('trainingLoop', 'course')])
    expect(groups[0]!.rows.map((r) => r.key)).toEqual(['trainingLoop', 'someNewThing'])
  })
  ...
  it('不改动调用方数组（排序是拷贝——原地 sort 会让上游的快照顺序随渲染变化）', () => {
    const input = [cv('trainingLoop', 'course'), cv('selfNode', 'singleton')]
    cardFamilies(input)
    expect(input.map((c) => c.key)).toEqual(['trainingLoop', 'selfNode'])
  })
})
...
describe('scopeBadge：只说 scope 说不出来的那件事', () => {
  ...
  it('course ⇒ 不挂徽章（按课程是默认语义，组标题已说；每行再挂一个只是噪声）', () => {
    expect(scopeBadge(cv('trainingLoop', 'course'))).toBeNull()

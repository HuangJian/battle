 ...
 *  - R13 ③：src/** 无 .tsx、无 preact import；dashboard/src/web/** 禁服务端 import；
...
import { renderConsolePage, renderLogPage } from '../src/web/render'
...
describe('日志页 SSR（render.tsx renderLogPage，§348 补 2 语义保留）', () => {
  it('follow 默认开：id="follow" checked 属性（不按裸词断言）', async () => {
    const { buildStateView, componentLogPayload } = await import('../src/server/api')
  ...
  it('follow=false 无 checked；缺文件显示占位', async () => {
    const { componentLogPayload } = await import('../src/server/api')

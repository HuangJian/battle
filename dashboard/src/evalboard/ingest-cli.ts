 ...
 *
 * 用法：
 *   bun dashboard/src/evalboard/ingest-cli.ts --eval-log <eval_log.jsonl> \
 *     --data dashboard/data/evalboard --run <run_id> --course <course> \
...
import { EVALBOARD_DATA_DIR } from '../core/paths'
...
export function runIngestCli(argv: string[]): { appended: number; duplicate: number } {
  ...
  const data = arg(argv, '--data', EVALBOARD_DATA_DIR)

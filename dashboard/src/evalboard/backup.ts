 ...
 *
 * 用法：`bun dashboard/src/evalboard/backup.ts --data <dataRoot> --out <dir>`
...
if (import.meta.main) {
  ...
  const data = at('--data') || EVALBOARD_DATA_DIR
  const out = at('--out') || path.join(EVALBOARD_DATA_DIR, 'backups')

 ...
 *
 * `ladder.json` 入库副本随代码（W3）；`dashboard/data/evalboard/ladder.json`
 * 是运行时工作副本（gitignore）。s1l3b0 的去基地 stage 在此冻结存 JSON
 * （不能只写"移除基地"四个字，§4.2）。
 *
 * 用法：`bun dashboard/src/evalboard/gen-ladder.ts [--write-data]`
...
if (import.meta.main) {
  ...
  writeFileSync(LADDER_CANON_PATH, text)
  console.log(`[ladder] wrote ${LADDER_CANON_PATH}`)
  if (process.argv.includes('--write-data')) {
    const data = path.join(EVALBOARD_DATA_DIR, 'ladder.json')
    try {

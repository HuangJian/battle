 ...
 *
 *  CLI（直跑本文件）：`bun dashboard/src/launch/cli.ts [--script <name>.py] [args...]
...
export function launchTraining(opts: TrainOptions): TrainLaunchResult {
  ...
  if (opts.check) {
    log('torch 可用。启动训练: bun dashboard/src/launch/cli.ts --script <name>.py [args]')
...
function usage(): void {
  console.log(`用法: bun dashboard/src/launch/cli.ts [--script <name>.py] [args...] [options]

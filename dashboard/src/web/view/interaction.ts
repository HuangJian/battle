...
/** 迟到的 `/api/state` 响应是否该**丢弃**（切课竞态）。
 *
 *  症状：快速 A→B 切课（或多课并行时来回点）时，A 的响应可能晚于 B 到达——它会把
 *  `stateView` 覆盖成 A 的数据，而侧栏/URL 已经停在 B ⇒ 面板展示**另一门课**的指标与
 *  走势，且下一次轮询才纠正（用户看到的是「切了但显示错课程的数据」）。
 *
 *  判据：请求发起时记录的查看课程 ≠ 此刻的查看课程 ⇒ 期间又切过课，这是更早那次的响应。
 *  空串（“未指定，跟随服务端生效课程”）照常参与比较：期间从空串切到某课也要丢弃。
 *  纯函数——它的每一条分支都值得单独钉住（拉请求无法在单测里复现的正是时序）。 */
export function isStaleStateResponse(want: string, current: string): boolean {
  return want !== current
}

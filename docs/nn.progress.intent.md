## §31. 后继方向确立 — 实施层缺陷 + 归档禁用项基线 + 无监督学习意图（2026-08-27 深夜）

**用户实机观看 intent NN 录像**，发现三类与 God-AI 共有的**实施层**缺陷（非意图选择层）：
① 冰冻期目标摇摆（无承诺）；② 不会清墙开路；③ 追击并行车道横向开火而非追尾。
用户结论：意图 NN 再训练也无法解决（学的是意图选择）；if-else 虽可表达但重蹈
"局部改善全局翻车"风险高（历史 20+ 阴性项）。

**记录位置**：`plan/Intent-Policy-NN-Plan.md` §12（后继努力方向，未跟踪不 git add）：
- §12.1 三缺陷根因分析表（huntCommitTicks 重评 / 顺路清墙 overlay / 追尾导航子分支）
- §12.2 归档禁用项（DECISIONS §166 starRush 等）= 改进对照基线：行为级测试 + 60-seed
  净收益门 + A/B 账本 + 既往阴性复查，缺一不放行
- §12.3 **用户设计意图**：开启 per-tick RL 的初衷 = 期望无监督学习克服实施层缺陷。
  诚实标注与 §0 张力：§0 判死的是"从零 RL 学 move/fire"（旧谱系 ≈10% 平台），用户意图
  是"意图框架约束下无监督微调实施层"（层级混合），**列为假设待验证**，执行前须可行性
  评估（动作空间收窄 / 实施层奖励 / 确定性兼容）
- §12.4 NN→规则反哺通道（差异桶取证 + 执行器库扩充），定位"可部署体更实用 + 可解释"
- §12.5 **意图条件化 action NN**：intent 之上再训更小独立 action NN 控制意图内走位/开火
  （用户确立）。绕开 BC 死因（走 RL）与旧谱系 ≈10% 平台（动作条件在意图上）；四门槛：
  执行层奖励设计 / 与执行器边界 / 推理预算 / 隔离纪律。最小探针 = HUNT 意图内追尾导航，
  一次性检验"执行层 per-tick 奖励给不给得出梯度"。
- §12.6 **人类录像利用通路**：意图层 SFT 已做有效（B′）；执行层逐帧 SFT = BC 死路禁；
  正确形态 = 人类作 RL 先验/自举（imitate-then-RL / KL 锚点）而非监督目标；补 s32-35
  人像（0 覆盖）优先；风险 = 人像分布窄微调过头（B 臂塌缩前鉴）。
...
## §33. M8 最终验收 — 5 权重 × 2100 局全量评估，it38 交付（2026-08-28 晨）

**评估**：hard，35 关 × 60 seeds = **2100 局/权重**，SE≈1.1pp，dist 派发全部 6 节点。
it33 首跑 87% error 作废重跑（§32 校验+重跑流程自动兜住）。全部 5 权重终版 **0 error**。
...
| it33 | 72.7% | +0.4pp |
| it36 | 71.2% | -1.1pp |
| it40 | 67.1% | -5.2pp（末段过冲确认，止损正确） |

**M8 结论**：
- **增益真实**：意图选择 RL 把 NN 从 72.3% 提到 75.0%（+2.7pp），意图多样性全程保住
  （熵 1.15+，8 类跨全）；it40 止损正确（确实退化 67%）。
- **超越未达成**：相对 God-AI ~76% 差 ~1pp（1 SE 带内）→ **下限目标"≈76% 不劣 +
  更小可推理"达成**；"超越"瓶颈 = 执行器天花板（M7① oracle 完美意图 = 76.6%，NN 75.0%
  距天花板仅 1.6pp——意图选择已近最优，天花板由词表×执行器定，非 RL 能抬）。
- **最差关 Battlement（s34）**：it38 8.3% / it33 15%，基地开局失守——后续所有方向的第一靶子。
- 交付权重 `nn-training/weights/intent-rl-weights.it38.20260828-000051.json`（备份 + it30/35/40 一并留存）。

**后继**：要超越 → 先抬执行器天花板（§12.5 action NN / §12.1 实施层缺陷 / §12.6 人类先验），
意图层才有新空间；或按"不劣"收官。

## §34. M8 收尾 + 候选方向优先级总表（2026-08-28）

**收尾声明**：Intent NN 任务 M8 阶段**收官**。交付权重
`nn-training/weights/intent-rl-weights.it38.20260828-000051.json`（75.0%）；权重归档完整
（it30–40 全留档）；结论：下限目标"≈76% 不劣 + 更小可推理"**达成**，"超越"未达成且已归因
（执行器天花板，非意图选择层问题）。

**候选方向优先级总表**（用户确立，全文见 `plan/Intent-Policy-NN-Plan.md` §13）：

| 优先级 | 方向 | 预期 | 触发 |
|---|---|---|---|
| **P1** | §12.1 执行器三缺陷修补（①冰冻承诺→②顺路清墙→③追尾） | +1~3pp | P0 取证后 |
| **P2** | §12.5 HUNT 追尾 action-NN 探针 | 0 或 +2pp 以上 | 与 P1-③ 并跑 |
| **P3** | §12.6 人像补料 s32–35 | +0.5~1.5pp | 视 P0 |
| **P4** | §12.4 NN→规则反哺 | 单独 ≈0 | 82% 实证后 |
| **P5** | M9 词表增长 | ≈0 | 挖到真词缺口 |

**不推荐**：继续 M8 续训（窄缝抖动，机时不如 P2）；直接开 M9（无在案缺口，大概率 Q9 三连停）。

**核心洞察**：P1 是唯一同时抬 God-AI / oracle / NN 三方天花板的方向（oracle 76.6% 是含三缺陷
的成绩）；P2 是唯一可能学出 if-else 表达不了行为的方向。两个不推荐项背后的判断 = PPO 已在
距 oracle 1.6pp 窄缝内，剩余增益不在意图选择层。
...
## §35 决策正文归档（搬自 `DECISIONS.md`，2026-09-23）

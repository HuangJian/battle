...
所以把上面那条纪律做成门禁：AST 扫 `tests/**/*.py` 里**每一处** `.sleep(...)`（桩子进程
源码是字符串，不算），要求同行尾注或上一行注释带 `# sleep-ok: <理由>`，且理由必须落进
**两族闭集**：
...
说明它在拿时长当同步。现况：33 处标注（17 文件），兑现流程顺带把 `test_priority_schedule`
的 `_stop_soon` 也改成事件驱动（等取消探针**真的问过一次**再停）。**不管辖**：

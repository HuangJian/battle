...
---
## §10 ragged tail 的**硬保证**：池子截到 mb 的整数倍（训练侧对齐，2026-09-23）

承 §8。§8 那条是**配置侧**的软保证（只在「每关都收满」时成立），真机第一次就破了 ——
所以补上训练侧的硬保证：**`chunk_episodes` 丢弃重排序列的尾部 `n % mb` 步**，让每个 chunk
恰为 `mb`。

### 真机读数（it98，TPU v5e，`x20-demo-mix`）

```
per-stage quota=12288: kept 47813 steps / 4 stages, dropped 2 shards
                        ; SHORT (供给不足) stages={2001: 1339}
12288×3 + (12288−1339=10949) = 47813
47813 % 1024 = 709        ← B=709 回来了（stage 2001 实测 ~195 样本/局，估的是 ~219）

B=1024: 184 步 / 墙钟 33s（0.179s/步）   ← 这才是 TPU 该有的样子
B=709 :   4 步 / 墙钟 50s（12.5s/步）    ← 83.8s 里 50s 买的是编译
4 个 epoch 各 21s（21/42/62/83 的差分）= 46 个快步 + 1 个 ragged 步
```

`累计编译=48s（新编译 8 次）/ 总墙钟=83s` ⇒ 每 epoch 2 次新编译（ragged 形状 + 被它挤出后再编的满块）。

### 为什么 §7 的持久化编译缓存救不了这一块

它管的是**跨进程冷启动**（每轮一个新 job 进程）；管不了**进程内 LRU**：ragged 形状每个 epoch
只用一次，中间隔 46 个满块步 ⇒ 必然被挤出、下一 epoch 必然重编。**形状不除，这笔钱月月交。**
...
* `n % mb == 0` 时**逐字节不变**（permutation 抽样顺序未动），且非整倍时前 k 块与旧输出逐字节相同
  —— 唯一变化是末块消失；
* `shuffle=False`（对照路径）与**单池**（`len(episodes) <= 1`，不重排 ⇒ 裁尾等于裁掉「局末」=
  有偏）刻意**不动**；池子 `< mb` 时保留一块（裁成 0 块 = 静默不训练）；
* 归一化粒度仍是「本轮整池」（含被丢的 `< mb` 步）—— 刻意保持既有语义。

代价：每轮少 ≤1023 步（本次 709 = 1.5%）。与它买回的 ~50s/轮（83.8s → 预期 ~35s）不在一
个量级，且这是**无条件**保证 —— 任何一关 SHORT 也不再产生第二个形状。

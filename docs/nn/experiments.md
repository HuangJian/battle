...
第一性原理审计确认旧表示缺"停"动作，设计错误的网络继续训练没有意义（用户裁定）。
H1/H2 未决，不判；verdict 移交新纪元（`plan/new-era-stop.plan.md`）。
起步核查通过（it1–3 kl 正常、kickstart=0、起点 sha 对上；it0 43.5 系种子子集+n=200 噪声，
it5 已回终点区）。数据封存 `tmp/x20-steady-cont/`，不复用。

---

## §43 x20-state-init 关账结算：判负（it1–154，日常反 хуже）（2026-09-26）

起始分布腿（人类中段快照注入，noexplore it105 起跑；预算 300 轮/12h，用户 154 轮手工停；
课程文件结算节已回填，本节是归档索引）。

- **日常**（860001，400 局/点）：it0 40.0/7.64 → it150 47.2/6.25（**比起点还差**）；
  全程 38–48% 震荡无趋势；p10 恒 1。训练内 rollout wr ~0.5（中段开局好打）与日常脱钩。
- **门控未跑**（it50 哨兵判死后按规则不烧判决； repertoire：M5 风险兑现——"见过≠会值"，
  V 外推不到没活过的世界）。
- 约束全绿（kl 均值 0.0124/max 0.0155、ent 0.42→0.60 无 break、超时 0）；复用度 ~40x（设计内）。
- **判负**：覆盖率路线未打出缺口；B1/B4 的 state-init 指望落空。附带产出三个 infra 修
  （混语料派发闸门 c4d42a04 / 纯本机路由 93f24719 / paired-kill 默认关火）。
  全文见课程结算节；数据 `tmp/x20-state-init/`。

---

## §42 x20-dodge-l2b 关账结算：判负，威胁塑形证伪（it1–66，门控未跑）（2026-09-26）

弹道速率势腿（唯一变量 `-wThreat*threatTicks/(ticks+1)`，wThreat=30；noexplore it105 起跑，
配对种子流；预算 300 轮/12h，用户 66 轮手工停）。

- **机制**：threatRate it0 19.5 → it60 21.3（/kt，平/反向）；clean 52.5→51.2%（平）；
  threat 自 it5 起六个点 20.9/22.0/20.9/21.1/19.5/21.6 全平——B2 升档条件 it15–20 即成立。
- **low%**：40.0/7.64 → 35.2/7.69（−4.8pp）；最优 it25 33.8，最差 it30 44.2（±5pp 震荡）。
  low 动而 threat 不动 = 撞大运条款，不认。
- 门控未跑（分布不动 + 震荡，门过先验低，不补测）。约束全绿；it62 rollout wr 0.0888 单点记录。
- **判负；B2 去向**：原判应升 wThreat=60，但全系列关账 + 新纪元已取代塑形路线，60 档不再开；
  按 plan §6 后半句，本路线证伪，转 DAgger/搜索蒸馏排队（新纪元结论后评审）。不转 state-init
  （154 轮已阴性）。全文见课程结算节。

---

## §41 x20-dodge-l2a 关账结算：判负（it1–65，门控未跑）（2026-09-26）

残血深度势腿（唯一变量 `-wDanger*max(0,0.4-playerHpRatio)`，wDanger=2.0；noexplore it105 起跑，
配对种子流；预算 300 轮/12h，用户 65 轮手工停）。

- **机制**：dangerRate 221 → 216/kt（平），threat 19.9→20.7（平）；clean 51.8→45.2%（反向）；
  唯一亮带 it15（threat −18%）未维持。专属死刑按字面成立。
- **low%**：39.3/7.76 → 36.0/7.41（−3.3pp 噪声）；最优 it40 31.0 spike 未维持（it55 反弹 39.0）。
- 门控未跑（机制零位移，门过先验低，不补测）。约束全绿。开局混杂声明：clean 反向 ⇒
  信号从未进入，混杂不背锅。
- **判负**：wDanger=2.0 无效。全文见课程结算节。

---

## §40 x20-dodge-l3d2 关账结算：判负，收入侧判死（it1–173 + 三候选门控）（2026-09-26）

收入侧剂量重试（`(wPickup,wStar) (3.5,2.0)→(4.0,4.0)` 并集腿，放弃项内归因；noexplore it105 起跑，
配对种子流；预算 300 轮/12h，用户 173 轮手工停，`loop-control.json` paused）。

- **机制**：拾取率 it0 76.5% → it170 86.0%（+9.5pp），峰值 it150 86.9%；it0–70 爬升后百轮横盘，
  距 90% 还差 ~4pp。星星 0.20→0.26/局（spawn 杀驱动，第二环仍断）。首轮同 it：it90 81.4→84.7%
  （剂量加倍只换 +3.3pp）。
- **low%**：40.2/7.65 → 37.2/7.75（−3.0pp 噪声）；区间 31.5（it165 spike，不追）~45.0；
  p10 34 点仅 8 个 ≥2。拾取涨、low 不跟 = B4 指纹。
- **414000 主段**（800局/权重，it25/100/130 + noex-it105 同命令配对；基线复测 33.0/8.33/p2 吻合）：
  it25 −1.4pp（p=0.57 打平）· it100 +7.5pp（p=0.0013 显著更差；日常最优反向标本）·
  it130 +2.2pp（p=0.36）。**门全灭**。415000 未跑（先验低，不补测）。
- 约束全绿（kl≤0.0164、ent 无塌、超时 0、cells 无塌）。实际 it1–173（预算 58%，~9.8h）。
- **判负；合并判语**：两轮剂量拾取 +5/+9.5pp、low 双双噪声、门控最好打平且一显著更差 ⇒
  **收入侧判死，不升 (5.0,6.0)**；下一步拾取行为探针。门控机时注记：self 全失败 + distLocal=0
  首跑 ~15局/分，定点清除后 `--dist-local 8` ⇒ ~400局/分（partial 保留 `.run.partial1`）。
  全文见课程结算节；数据 `tmp/x20-dodge-l3d2/settle/judge-414000.jsonl`（配对键 stageId）。

---

## §39 x20-dodge-l1d2 关账结算：判负，L1 方向判死（it1–31 + 三候选门控）（2026-09-26）

开局闪避腿剂量重试（唯一变量 `−wOpen*dmgFirst600`，wOpen=0.02；noexplore it105 起跑，
配对种子流；预算 300 轮/12h，用户 31 轮手工停，`loop-control.json` paused；in-flight it32
rollout 废弃，无 it33）。

- **机制（日常，腿内趋势）**：it0 真基线 52.5%/71.0（n=200 bc）→ it30 51.2%/69.4
  （Δ−1.3pp/−1.6，噪声）；唯一亮带 it15（58.2%/62.7）未维持；it20 全程最差
  （48.5%/78.0）。专属死刑（连续 3 点不动即停）按字面成立。
- **日常 low**：40.0/7.64 → 37.2/7.82（Δ−2.8pp 日常）；最优 it10 35.8（−4.2pp），
  it20 崩到 39.2/7.01。
- **414000 主段**（800局/权重，it10/25/30 + noex-it105 同命令同 seed0 四权重配对；
  noex-it105 复测 33.0/8.33/p2/win18.1，与档案逐字吻合）：it10 38.4/7.26/p2
  （+5.4pp，McNemar 154/197 p=0.025 显著更差）· it25 37.0/7.77/p2（+4.0pp，
  p=0.086）· it30 35.5/7.92/p1（+2.5pp，p=0.32）。**门全灭**。分关无亮点
  （it10 s2000 29.5→48.0 崩）；早死占比 28.7→29~34.5% 无改善。
- **415000 回测**（noex-it105 36.9/7.44/p1，与档案吻合）：it10 +2.5pp ·
  it25 +1.8pp · it30 −3.4pp（p=0.15 不显著）。回测改善＋主段恶化＝跨段不稳定，
  与首轮同构，开局闪避未习得。
- 约束全绿（mean≥7.0、超时全 0、kl≤0.013、ent 0.54~0.66、cells 无塌）；
  dmg/局 243→254（全局换血亦不动）。
- **合并判语**：两轮剂量 (0.01, 0.02) 机制双双零进展、门控七候选全灭 ⇒
  **L1 方向判死，不开 wOpen=0.03**；B1 不触发。数据
  `tmp/x20-dodge-l1d2/settle/judge-414000.jsonl` + `backtest-415000.jsonl`
  （基线同命令配对，比首轮结算更强）。
  配对方法勘误：首轮口径复算须用 `(stageId, seed)` 键，裸 `stage` 字段不存在，
  错键会把 800 行压成 200 行出伪读数（本次结算中已更正，结论不变）。

---

## §38 x20-dodge-l3 早停分析：判负倾向，停腿合理（it94，eval 到 it90，门控未跑）（2026-09-25）

收集行为腿（wPickup 3.0→3.5 + wStar 2.0 并集，noexplore it105 起跑，与 L1 并行同种子流；
预算 300 轮，用户 94 轮手工停）。

- **机制（拾取率 frozen 口径，可配对）**：真基线 76.3%（pu 2.06/sp 2.70）→ it90 81.4%
  （+5.1pp），峰值 it85 83.8%；it0–55 爬升后 35 轮横盘，距 90% 门槛还差 7~9pp。
  vs noex 前半程 ±2pp 无差、后半程 +3~5pp（弱效应）；星星 0.19→0.23 但 spawn/局
  2.70→2.65 没涨——wStar payoff 链“多杀→多 spawn”第二环即断。
- **low%**：40.0/7.64 → 38.8/7.48（−1.2pp 噪声）；vs L1 同 it 全程不优（0~+7pp）。
- PPO 健康、约束全绿；门控未跑且不补测（门过先验低）。
- **判负倾向；下一步升中档 (4.0, 4.0) 重开或先做拾取行为探针**。全文见课程结算节。
- 附带：两腿 it0 基线污染同源 infra bug（见 §37），L1 结算 it0 已更正为真基线。

---
## §37 x20-dodge-l1 关账结算：判负（it1–300 跑满 + 四候选门控补测）（2026-09-25）

开局闪避腿（唯一变量 `−wOpen*dmgFirst600`，wOpen=0.01，开局窗双倍定价；noexplore it105 起跑，
配对种子流；预算 300 轮/12h，实际 ~5.7h；09:39 在线 kind=iter 两次 ABORT（节点缺 bun）
后转离线跑满）。

- **机制（日常，腿内趋势——历史臂 pre-v8 无 dmg 字段，无配对基线）**：it0 真基线 52.5%/71.0
  （n=200 bc；此前误用被污染的 53.0%/68.5，见 infra bug）→ it300 51.0%/68.9，零进展；
  唯一亮带 it55~65（clean 58~62%/dmg 58~61）未维持。
- **日常 low**：it0 真基线 40.0/7.64 → it300 33.8/8.09（Δ−6.2pp 日常；门控段全灭，判负不变）。
  配对段仅 it55/60/65 显著且与机制亮带同窗。
- **414000 主段**（800局/权重，it60/105/255/285 单命令同 seed0 跨权重配对；noex-it105 复测
  33.0/8.33/p2 与档案吻合）：it60 32.9/8.52/p2（−0.1pp）· it105 37.8/7.54/p2（+4.8pp，
  McNemar p≈0.04 显著更差）· it255 34.2/8.21/p2 · it285 32.8/8.55/p2。**门（−3pp 且 p10≥2）全灭**。
- **415000 回测**（noex-it105 36.9/7.44/p1）：it60 31.2/8.05/p2（−5.6pp，p=0.015）·
  it255 31.1/8.66/p2（−5.7pp，p=0.016）· 其余边缘不劣。回测改善＋主段零改善＝跨段不稳定，
  开局闪避未习得（真学会应两段同向，开局机制与段无关）。
- 约束全绿（mean≥7.35、超时 0、kl≤0.016、cells 无塌）；playerDamageTaken/局 249→243。
- **infra bug（2026-09-25 追查）**：两腿 eval_log it0 400局 = 200局 bc真基线 +
  200局后期权重误标 it0（L1 混入 it43 bf27，L3 混入 it51 f8ab；均为重启时 it0 重评估
  读了 live 权重文件）。污染仅 it0，其余 iter 单 wver 干净；门控 verdict 不受影响。
  跨 run 确定性本身完好（同 wver 同种子逐局 100% 一致）。
- **判负；B1 不触发**（短暂联动后回落 ≠ T2/T3动+low不动）；下一步按剂量梯升 wOpen=0.02
  （课程 §4 剂量重试）。教训：① 开机制腿前先 resim 对照臂 v8 基线，否则机制无配对；
  ② judge 行无 v8 列，门控段机制天然不可测；③ 日常↔门控 rank 不一致（it285 日常还行、
  门控平庸），日常不能代餐。全文见课程结算节；数据
  `tmp/x20-dodge-l1/settle/judge-414000.jsonl` + `backtest-415000.jsonl`。

---
## §36 闪避 Phase 2 过程指标对照表：L1 验证通过，L2 口径必须改（2026-09-24）

metrics v8 四列的第一张对照表（人类 62 replay 本地 resim，`tmp/human-v8.ts`，mism 0；
NN 三权重同 400 局/860001 本地贪心 resim，`tmp/mirror-probe.ts` + v8 累加；
人类 dmg mean125/p50 100 与审计逐数一致，保真独立复核通过）：

| | dangerTicks（均值/p50/千t速率） | threatTicks（均值/p50/速率） | dmgFirst600（均值/p50/p90） | clean600 | pass |
|---|---|---|---|---|---|
| 人类 62 | 868/**0**/175 | 48/36/9.8 | **17**/0/100 | **85%** | 100% |
| noexp105 | 564/171/271 | 36/24/17 | 72/0/214 | 52% | 15.5% |
| steady175 | 498/148/257 | 40/23/21 | 93/86/244 | 41% | 14.5% |
| x7best | 480/184/275 | 41/26/24 | 97/100/234 | 36% | 6.5% |

三条结论（Phase 3 立即生效）：

1. **L1（dmgFirst600）按原样验证通过**：均值差 4~6×、clean600 85% vs 36~52%；
   固定 600t 窗消除了存活时长混杂（NN 早死局窗口更短还更差，反向偏置下依然显著）。
2. **L2 不得用原始累计列入公式**：danger 原始均值人类（868）> NN（~500）——活得久、
   扛着残血打完的局累计更多，罚原始值 = 罚“在危险里活下来”，方向反了；threat 原始值
   人类（48/36）≥ NN——人类主动进弹道开火，罚原始值 = 罚接战。L2 落笔必须换**速率口径**
   （danger 175 vs 257~275/千t ≈1.5×、threat 9.8 vs 17~24 ≈2×，分离成立）或 threat 加
   “未还击”门控；原始列降级为诊断。plan §4 L2 行待用户批准后改。
3. **附带**：steady175 的过程指标全面不优于 noexp105（dmg600 反而 93>72）——B 腿
   “稳”没稳到过程上；x7best 开局最糙（clean600 36%），c07 训练从未给开局定价。
   另：远端节点（mac/a95）仍跑 pre-v8 代码，云 eval 的 v8 字段全 0（汇总 `0/0` 诚实
   示空）——云节点重推前，过程读数只以本地 resim 为准。

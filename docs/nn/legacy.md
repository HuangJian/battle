# 早期谱系归档（2026-08-18 ~ 08-29）

> v1/v2 student、P1.5 蒸馏、BC 热启动、无道具纪元、第一代 RL 流水线——已被 goal-space 取代，只作史实与教训保留。
>
...
---
## §22 goal-space 策略网络重建开工（2026-08-29，M9 时代）
...
### 21.3 三节点验证（mac 192.168.0.88 / a97 192.168.0.97 / a98 192.168.0.98）
...
### 21.4 m1-eval v3.8 混合分派（用户指正 2026-08-27）
...
### 21.5 scheduled sampling BC（M5 补件）训练验证（2026-08-27）
...
## §20 M3 BC warm-start 双臂（2026-08-26，plan/AI-No-Items-Warmstart.md §6）

### 20.1 训练（纯 BC，value 预置降级说明）
...
### 20.3 Gate（m1-eval --policy nn，hard 35×10 贪心，350 局真实仿真）
| 臂 | WIN | suite v7 | 说明 |
|---|---|---|---|
...
- **按 plan §6 Gate 规则 → 「≈0%：不走 M4，回环整改」**：已完成一轮完整回环
  （纯 BC A/B → DAgger 混合），三条线均 0%——见 §20.4 结论与决策点。
...
## §19 语料纪元 OBS_SCHEMA_MAJOR 1→2（2026-08-26 凌晨，plan/AI-No-Items-Warmstart.md M2）
...
## §18 无道具纪元开启：M0 + M1（2026-08-26 凌晨，plan/AI-No-Items-Warmstart.md）

> 用户拍板「全部 AI 不使用主动道具 + RL 预热」路线；执行 M0（摘除道具）→ M1（分歧探针）。
> §19 记录 M2 语料纪元（OBS_SCHEMA_MAJOR 1→2）。

### 18.1 M0 God-AI 摘除主动道具（DONE，已 commit）
...
## §17 补丁：eval 本地参与（PPO 收尾后本机算力入列）（2026-08-25 傍晚）
...
## §16 R6 破局三件套落地 + 评审修订与重启记录（2026-08-25 下午）

> §15 审计定位 loss-band 局部最优后，实施三方向改动（奖励重做 / 课程化 / PPO
> 收紧），顺带修复两个潜伏奖励 bug。本节含独立评审结论、执行修订（A/B/C）与
> R6 首次启动参数——旧谱系数据已归档，见 §16.4。
...
## §15 收敛性审计：it1-68 平台期定量画像（2026-08-25 午后）
...
## §14 streamKlCap 预算重标定 0.12→0.20（2026-08-25 午）
...
- §12 已记录熔断自流式上线起 33/50 轮静默触发；R2 的「sps 4→3 贴预算」估算
...
### 13.3 测试（test_run_rl.py 常驻，禁临时脚本）
...
## §12 流水线衔接优化：KL 遥测补盲 + 熔断止损 + dist/eval 解串（2026-08-25）
...
## §11 干净评估嵌入流水线（2026-08-24 晚，DECISIONS §283）
...
### 11.4 流式模式时序缺陷（实跑发现，21:05 修复）
- **现象**：it23 rollout 收官后始终无 [eval] 行。根因：§11.1 的钩子位置假设
...
## §10 训练可观测性落地 + 权重逐轮归档 + mb1024/workers10/self-agent2 重启（2026-08-24 早）

> 背景：it2 的 PPO 阶段跑了 **110 分钟全程零输出**（旧代码只有首尾两行日志），
> 任务管理器只见 python ~50% CPU，无法判断进度与健康。用户要求阶段性日志。

### 10.1 可观测性（纯打印，零 RNG/数值影响）
...
- 每次 PPO 写回后 `shutil.copyfile` 至 `nn-training/weights/rl-weights.it<N>.<YYYYMMDD-
  ...
  `weights.<ts>_ep<N>_val<V>` BC 自动发现正则（§9.3 手工备份同理由）；
  glob 清理只认 `rl-weights.it*.json`，手工备份永不被删。tmp 合成数据单测通过。

### 10.3 重启（用户指令：mb 512→1024、workers 2→10、本地 agent --workers 2）
...
### 10.6 报告口径修复 + 收尾智能分发（tail dispatch）（2026-08-24 午后）
...
- **收尾智能分发（tail dispatch）**：此前中央 deque 自由竞争，慢节点可能在
  ...
  洗牌确定性不变。**下次重启训练生效。**
- **流水线提案分析（rollout/PPO 重叠）**（→ **被 §10.7 推翻**：否决理由中的
  "语料跨版本"不成立，见 §10.7 的修正）：①前提纠正：当前 rollout 完全结束后才进
  PPO，PPO 独占整机；~50% CPU 是小模型+串行分发的 torch 天花板（§7.10），不是被
  采样抢占，减 worker 无益。②~~严格依赖链……~~（误判：轮内权重本就冻结）。
- **实际可用的提速杠杆仍是缩短 PPO**：epochs 4→3（线性 -25%，KL≈0.02 远低于
  0.08 有富余）或继续观察 mb=1024 后的 KL 再定。

### 10.7 流式迭代落地（--stream 1）：推翻 6.6 的流水线否决（2026-08-24 下午）
...
### 10.5 rotate 抽签失配事故：it5 整轮重跑（2026-08-24 晚）
...
## §9 队列模式静默跳轮事故复盘 + 修复 + 重启（2026-08-24 凌晨）
...
4. start-training.ps1 detach 分支 stdout/stderr → `tmp/run_rl-<stamp>.{out,err}.log`
   （编辑后 BOM 被剥，已手工补回——§6 同坑三踩，教训再次确认）。
...
## §7 RL 阶段设计（承接 P1.5 蒸馏，2026-08-21）
...
### 7.4 Rollout 回路（TS 导出 + Python 训练，沿用范式）
...
### 7.6 接手验证：RL 链路端到端冒烟全通（2026-08-21 接手）
...
(c) 奖励权重初值（W_WIN=5/W_KILL=0.2/W_BASE=1.0/W_SURV=0.01）按 §7.3 待首轮 reward
曲线监控后定稿。

### 7.13 R3 实施：奖励与 godai-score v7 全面对齐 + 维度遥测（2026-08-21）
...
### 7.14 R3 长跑失败复盘：秒投降坍缩 + F3/F3b/F4 修复（2026-08-22）
...
### 7.15 R4 无限长跑启动 + detach 后台化修复（2026-08-22）
...
- **启动器缺口修复**（start-training.ps1）：detach 分支原硬编码只认
  ...
  BOM → PS 5.1 GBK 误读中文注释 ParserError（§6 同坑二踩），手工补回
...
### 7.16 R4 首次健康度巡检：进程环境性死亡，续训重启（2026-08-22）
...
### 7.17 keep-iters 3→5 无损切换 + 巡检：可胜关卡 6→16（2026-08-22）
...
### 7.12 决策：不做 rollout/PPO 流水线，瓶颈转向 PPO 自身（2026-08-21）
...
### 7.11 评审处置：rl-mimo.review.md 确认项全部落地（2026-08-21）
...
### 7.10 PPO 提速排查：安全杠杆无效，mb 翻倍是正解（2026-08-21）
...
- **真正杠杆：mb 256→512**。梯度步 270→135/轮 → PPO ~5.5min→~3min，一轮
  ...
  的更新 = 每轮 KL 漂移更小，与 §7.7 的 KL 偏高顾虑同向，属双赢。

训练已用新配置重启（自 it4 权重 resume，轮番方案不变）。

---

### 7.9 正式训练启动 + rollout 并行化（2026-08-21）
...
### 7.8 启动脚本收敛：run_rl.sh 下沉为 run_rl.py（2026-08-21）
...
整步被静默跳过（正是 §7.6-(a) 预警的形态）。已修：`set -eu` + 循环前建目录。
...
## §6 启动器 Windows 本机验证：ps1 双 bug 修复（BOM + 参数风格兼容，2026-08-21）
...
## §5 移动死锁修复 + 0% 根因收敛（续 §2，2026-08-20）

### 关键修正：policy-input.ts 的 move-freeze 死锁（§4 评估 0% 的真正主因）

§4 报告「学生 BC 12ep → avgKills=0 / avgTicks=3933」与「+DAgger 6ep 仍 0%」。
...
ready=true 时 `fireLogits[0] >> [1]`，fire 命中仅 3/477）。这与 §4 的
「BC 分布漂移」根因一致，但本次是更纯粹的**学生自部署漂移**（教师只在教师
状态上标过 fire）。

### 对 §4 DAgger 回合的影响（重要）

§4 的 DAgger 冒烟（9 局 3725 样本）是在 **freeze bug 存在时**采集的——学生
...
## §4 P1.5: God-AI 教师端到端蒸馏管线（学生架构）验证 (2026-08-20)
...
## §3 v2: Scalar Fusion Architecture (2026-08-19)
...
## §2 v1: Conv-Only Baseline (2026-08-18 → 2026-08-19)
...
旧 §-1→§1 · 旧 §0→§2 · 旧 §1→§3 · 旧 §2→§4 · 旧 §2.1→§5 · 旧 §2.2→§6 · 旧 §3→§7 · 旧 §4→§8 · 旧 §5→§9 · 旧 §6→§10 · 旧 §7→§11 · 旧 §8→§12 · 旧 §9→§13 · 旧 §10→§14 · 旧 §11→§15 · 旧 §12→§16 · 旧 §12.5→§17 · 旧 §13→§18 · 旧 §14→§19 · 旧 §15→§20 · 旧 §16→§21 · 旧 §17→§22

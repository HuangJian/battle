...
* **torch_xla 官方文档**几乎逐字支持 §133/§134 的修法：编译产物会被缓存复用；`torch_xla.sync()`
...
## §134 真机验证：修复后 PPO 单轮 1200s+ → **88.9s**（2026-09-22）

承 §133：`demo_index` 复用缓冲进包后（ipynb 重打包），真机 `x20-demo-mix` it2：
...
## §133 根因定案：demo 混 batch 的 host 索引让 XLA **每步重编译**（2026-09-22）

承 §132 的观测（诊断行上真机即定案）。真机读数（`x20-demo-mix` it1，B=1024 恒定、kl1/demo1 恒定）：
...
而一次这样的编译就是秒级（§132 实测 7.7s）。这就是「离线 8~10s/步」的全部。
...
## §132 TPU 步耗诊断进日志：把「8~10s/步」从猜测变成读数（2026-09-22）

承 §131 与 `test_tpu_backend_guard.py`：Kaggle v5lite-8 离线课程 PPO 单步 8~10s，而
...
## §124 rollout/eval 优化落地：native features 内核进生产（plan/rollout-eval-opt.plan.md，2026-09-21）
...
## §117 探针 A 补强结果：God-1命 在本段崩盘 seed 上 low% 46.9% ⇒ 开局桶【不封存】，第二轮"环境致死"判断被推翻（2026-09-21）
...
## §116 x20 判决段解剖数据归档入库：docs/x20-dissection-414000.md（2026-09-21）

**动机**：§110–§113 的实测全部落在 tmp/（随时被清理）。已把三臂配对原始数据（NN-1命 it175
判决段 800 局 × NN-2命 × God-2命，414000 段）+ §113 三桶名册 + it30 时代 20-seed 配对
...
对既有章节的回填：§113 的 seed 名单指针（原 `tmp/x20-steady/dissection-414000-seeds.json`）
⇒ 归档文档附 B；§111/§112 的产物 jsonl 同理。复核新数（与 §113 一致或补强）：低分局死亡
中位 736t（§113 记 735，逐 seed 重算值）；mid 死亡中位 1569t（§113 记 1559，同为中位口径
微差）；clutch 桶 NN-2命通关率 67.5% / God-2命 65.9%（§113 的"89% 至少一方可转化"为并集口径）。
...
**已被 §117 的本段全量实测推翻**（那 10 个 seed 是 it30 崩盘集，与 it175 的 275 局不同 pop）。
**已完成（见 §117）**：414000 段 God-1命 800 局 × 附 B 名册配对——275 开局事故 seed 上
low% = **46.9% < 60% ⇒ 开局桶不封存**；God-1命 全段 low 50.2% / pass 16.5% / p10=0。

---


## §115 C 双臂停腿结算：无结论 —— kk=1 洗掉起点，C-0 it58 云机失联（2026-09-21）
...
## §114 C 腿评审修双机制：阶跃改斜坡 + 配对改代码保证（2026-09-20）
...
## §113 判决段解剖（414000 三份配对）：低分局 97% 是开局事故，金矿在残局+中段（2026-09-20）
...
- **C 腿靶子定稿**：clutch 123 + mid 272 = 395 局（~50% 败局）加权，275 开局事故剔除，
  ...
  （原 `tmp/x20-steady/dissection-414000-seeds.json` 已归档于此，见 §116）。
  （关键备注：这是"给可学局加权"，不是"给低分局加权" —— raw 低分加权会把 1/3 权重
  倒进开局事故，是 C 腿第一个坑，已绕开。）

---

## §112 God 两命对照（414000，800 局）：NN 追平均值、下限反超，差在通关转化（2026-09-20）
...
## §111 两命对照：加一条命，低分局 35.5→1.6 —— 低分局≈开局事故（2026-09-20）
...
## §110 x20-steady 终点判决（it175）：门边未过 —— 35.5 vs 35，差 4 局（2026-09-20）
...
## §109 x20-steady 停腿结算：B 有效但不足，且"稳"未被分离证明（2026-09-20）
...
- **两阶段曲线**：it0–85 横盘震荡（low% 37–48 无趋势，it40 检查点 44.2 若执行会误杀）；
  ...
  但 414000 段一直有 ~6pp 落差（§108），终点权重尚未判决 —— 结算差最后一跑。
...
## §109 引导期大 body 护栏 + wire 账聚合：M1（低速重抽）的收尾（2026-09-20）
...
- `tools/wire_report.py` 对现场原文**实跑**：`job:payload` p50 3.9 s / p90 **321.8 s**、worst 11 KB/s、
  bad% 50%、reroll 1（正是 §105 那两次坏签的形状）。
...
## §108 x20-steady it135 判决 + 回测：回测门过，判决门差约一半（2026-09-20）

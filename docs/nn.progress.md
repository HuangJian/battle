...
⑦ 作业工作区/TAR/git 物化（`REPO_ROOT`/`JOB_DIR_KEEP`/`_persist_result`/`prune_job_dirs`/`unpack_opt_tar`/`pack_opt_tar`/`unpack_payload_or_fail`/`_git_head`/`_ensure_commit`）→ `remote/job_fs.py`（BC 拆出的前置：`_persist_result` 非 BC 专属；五刀里唯一 **seam-free**——全仓直接调用无 `setattr`）；顺手发现 `_ensure_commit` 全仓零调用（既存死代码，已登记未删）（2026-09-23） |

...
| §149 | docs/nn/remote-transport.md §49 | ❌ ~~第三块盘 `battle.cloudflared.ipynb` 明确退役~~ **该裁决已撤回**（它走 cloudflared 公网隧道，与 tailscale 盘是两条**不同网络环境**的接入方式，不冗余）；P4 真正范围 = **去掉两块盘对 bun 的依赖**（取向待裁）（`DECISIONS §2026-09-25-goalnn-cloudflared-notebook-retired【已撤回】`）（2026-09-25） |

...
组件可能从未启动」。根因双叠：① 正在运行的控制台进程早于 §374 动态查找提交
（一直未重启，加载的是旧代码 → 静态路径全部失效）；② 首页组件表（componentViews）
仍只用静态 COMPONENT_LOGS 映射 + 账本回退，cloudflared 动态文件名
（cloudflared-<ts>.log）在账本缺失/被清理时查不到 → 首页日志尾空 + 径错。

决策（保持日志解析单一来源）：
1. **组件表与 /log/<key> 同源**：componentViews 的 log/logTail 一律走
   `resolveComponentLog`（静态 → 账本 → 动态查找），删掉第二套静态解析。
2. **scanLatestLog(dir, key, course) 参数化**：扫描目录可注入（真实路径 LOG_DIR，
   单测注入临时目录获得确定性）；mtime 最新为准，只扫一层不递归（§366 教训）。
   动态查找天然解耦启停顺序——每条 /api/log 请求重新解析，组件启动后 4s 轮询
   日志即出现，与「先起控制台」「先起服务」的顺序无关。
3. **账本真实性回归**：registry 出现与事实不符的脏条目（pid 1/7 + 旧退出标记，
   疑似先前调试残留）时，UI 对活进程显示「已退出」假象——按真实进程 pid 重登记
   （hubServer = hub_server 进程、trainingLoop = run_rl 进程），控制台重读即真相。

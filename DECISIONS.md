...
- **仍未做（明确记录）**：① `tests/test_push_bootstrap_teardown.py` / `test_worker_server_lock.py` 仍用本文件私有的 `_free_port`，但它们的 `bind` 发生在**进程内**（窗口微秒级、不跨进程冷启动），本轮刻意不动；② 平台侧的正解仍是让服务支持「`--port 0` ⇒ 自选端口并回报」——hub/worker 现在都要求显式 `--port`，那是产品面改动，不属测试修复。

...
多步计划机制（计划 §6 停止条件方向）。
> 全文 → docs/god-ai-tuning.progress.md §277

## 278. all-on−firingLaneMode CMA-ES 两轮收口 — 数值调参止于 −10.4pp，方向关闭 (STATUS: 完成)

**Decision:** 按用户指令（§277 后：关掉头号元凶 firingLaneMode，其余全开跑 CMA-ES），
...
**Rationale:**
- 关掉 FLM 后 all-on 从 42.9% 回升至 62.7%（+19.8pp，与 §277 LOO 结论一致）；但
...
**Implications:** 「开启被否策略」方向正式关闭（§277 门禁 + §278 数值双否决）。
`--profile all-on-minus-flm` 与 CMA-ES 工具链保留作复测入口；后继转向状态表示 /
多步规划机制（计划 §6 停止条件方向）。
> 全文 → docs/god-ai-tuning.progress.md §278
...
- 原地重试安全性由三层断点（§281）保证：resume 保留已完成 shard + PPO ckpt，不重跑已完局。
